...
import {LayoutDashboard,FileText,UploadCloud,ScanLine,Search,Sun,Moon,Plus,ArrowUpRight,CheckCircle2,AlertTriangle,XCircle,MoreHorizontal,ChevronRight,Database,Activity,Menu,Download,Filter,Pencil,Trash2} from 'lucide-react';
...
function PolicyDetails(){
  ...
  <div className="section-heading"><div><h2>Extracted controls</h2><p>Rules parsed from the policy document and ready for evaluation.</p></div><div className="inline-actions"><Button icon={Plus} onClick={()=>{setEditing('new');resetDraft()}}>Add control</Button><Button icon={Download}>Export</Button></div></div>
...
function Results(){
  ...
  return <section className="page"><PageTitle eyebrow="Scan results" title="Compliance evaluation" description={`${scan.policy_name} · Completed ${fmtDate(scan.run_at)}`} action={<Button icon={Download}>Export report</Button>}/>
